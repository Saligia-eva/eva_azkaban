...
public class ProjectManager {
  ...
  private final Permission defaultPermission = new Permission();;
  ...
  public ProjectManager(ProjectLoader loader, Props props) {
    ...
    // 设置默认权限
    String strDefaultPersmission = props.getString("project.default.permissions", "");

    if(strDefaultPersmission.trim().length() == 0){
      defaultPermission.setPermission(Type.ADMIN,true);
    }else {
      List<String> permissions = Arrays.asList(strDefaultPersmission.split(","));

      if(permissions.contains("ADMIN")){
        defaultPermission.setPermission(Type.ADMIN, true);
      }else{
        if(permissions.contains("READ")){
          defaultPermission.addPermission(Type.READ);
        }
        if(permissions.contains("WRITE")){
          defaultPermission.addPermission(Type.WRITE);
        }
        if(permissions.contains("EXECUTE")){
          defaultPermission.addPermission(Type.EXECUTE);
        }
        if(permissions.contains("SCHEDULE")){
          defaultPermission.addPermission(Type.SCHEDULE);
        }
        if(permissions.contains("METRICS")){
          defaultPermission.addPermission(Type.METRICS);
        }
      }
    }
  ...
  public Project createProject(String projectName, String description,
      User creator,  HashSet<String> validGroup) throws ProjectManagerException {
    ...
    if (creatorDefaultPermissions) {
      ...
      * 增加　用户组
       */
      // 只有一个用户组则给添加, 否则则放弃添加用户组
      /*
      * 获取用户组的权限
       */

      if(validGroup.size() == 1){
        String groupName =validGroup.iterator().next();
        projectLoader.updatePermission(newProject,groupName , defaultPermission, true);
        newProject.addProxyUser(groupName);
      }
