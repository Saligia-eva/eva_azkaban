...
public class TriggerManager extends EventHandler implements
  ...
  private class TriggerScannerThread extends Thread {
    ...
    public void run() { // 运行检查　trigger -- 一分钟轮询一次
      while (!shutdown) {
        synchronized (syncObj) {
          try {
    ...
    private void checkAllTriggers() throws TriggerManagerException { // 启动调度
      ...
      for (Trigger t : triggers) {
        try {
          ...
          // triger 准备完成
          if (t.getStatus().equals(TriggerStatus.READY)) { // trigger 准备完成
    ...
    private void onTriggerTrigger(Trigger t) throws TriggerManagerException {  // 调度过程
      ...
      for (TriggerAction action : actions) {
        try {
          ...
          if(currentCheckTime > 0){ // 获取最大的检查时间　
      ...
      if (t.isResetOnTrigger()) {   // 重新设置下次检查时间
      ...
      try {
        ...
        triggerLoader.updateTrigger(t);  // 将调度信息刷新到 trigger 表中 -- 更新内容包括 : 下次检查的时间, 还有
      } catch (TriggerLoaderException e) {
        throw new TriggerManagerException(e);
      }
    }
