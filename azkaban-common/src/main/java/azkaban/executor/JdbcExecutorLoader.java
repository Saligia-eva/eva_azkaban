...
public class JdbcExecutorLoader extends AbstractJdbcLoader implements
  ...
  // 加载所有的　backup 状态的　executor

  @Override
  public List<Integer> fetchAllBackupExecutorIds() throws ExecutorManagerException {
    QueryRunner runner = createQueryRunner();
    FetchBackupExecutorHandler backupExecutorHandler = new FetchBackupExecutorHandler();

    try{
      List<Integer> executorIds =
              runner.query(FetchBackupExecutorHandler.TETCH_ALL_BACKUP_EXECUTORS, backupExecutorHandler);
      return executorIds;
    }catch (Exception e) {
      throw new ExecutorManagerException("Error fetching backup executor id", e);
    }
  }

  // 加载所有的　user 到executor 的映射关系
  @Override
  public HashMap<String, HashSet<Integer>> fetchAllExecutorUsers() throws ExecutorManagerException {
    QueryRunner runner = createQueryRunner();
    FetchExecutorUserHandle executorUserHandle = new FetchExecutorUserHandle();

    try{
      HashMap<String, HashSet<Integer>> userExecutors =
              runner.query(FetchExecutorUserHandle.FETCH_ALL_EXECUTOR_USERS,executorUserHandle);
      return userExecutors;
    }catch (Exception e) {
      throw new ExecutorManagerException("Error fetching executor users", e);
    }
  }
   ...
   *@author dongtao
   *@param groupName
   *@return env param(key, value)
   */
  @Override
  public HashMap<String, String> fetchAzkabanEnvByGroup(String groupName)
          throws ExecutorManagerException{
    QueryRunner  runner = createQueryRunner();
    try{
      return runner.query(AzkabanEnvPros.GetEnvByUser, new AzkabanEnvPros(), groupName);
    } catch (SQLException e) {
  ...
  private static class AzkabanEnvFlag implements ResultSetHandler<Boolean>{

    private static String GetEnvFlag = "SELECT yarn_back FROM azkaban_group_manager WHERE group_name=?";

    @Override
    public Boolean handle(ResultSet rs) throws SQLException {

      if(rs.next()){
        return rs.getBoolean("yarn_back");
      }
      return null;
    }
  }

  private static class AzkabanEnvPros implements ResultSetHandler<HashMap<String, String>>{

    private static String GetEnvByUser = "SELECT a.property_key,a.property_value FROM azkaban_flow_properties a join azkaban_group_manager b on b.yarn_flag_name = a.property_user where b.group_name=?;";

    @Override
    public HashMap<String, String> handle(ResultSet rs) throws SQLException {
      HashMap<String, String> envPros = new HashMap<String, String>();
      while(rs.next()){
        envPros.put(rs.getString("property_key"), rs.getString("property_value"));
      }
      return envPros;
    }
  }
  ...
  /*
  *  JDBC 通过　executor_user　表查询userName 与　id 的映射关系
   */
  private static class FetchExecutorUserHandle implements
          ResultSetHandler<HashMap<String,HashSet<Integer>>>{
    private static String FETCH_ALL_EXECUTOR_USERS =
            "SELECT username, exec_id FROM executor_user";

    @Override
    public HashMap<String, HashSet<Integer>> handle(ResultSet rs) throws SQLException {
      HashMap<String, HashSet<Integer>> executorUsers = new HashMap<String, HashSet<Integer>>();

      while(rs.next()){

        String userName = rs.getString("username");
        int execId = rs.getInt("exec_id");

        HashSet<Integer> executorIds = executorUsers.get(userName);

        if(executorIds == null){
          executorIds = new HashSet<Integer>();
          executorUsers.put(userName, executorIds);
        }

        executorIds.add(execId);
      }

      return executorUsers;
    }
  }
  ...
  *   JDBC 查找备用的　executor
  ...
  private static class FetchBackupExecutorHandler implements
     ResultSetHandler<List<Integer>>{
    private static String TETCH_ALL_BACKUP_EXECUTORS=
            "SELECT id FROM executors where type=1";
