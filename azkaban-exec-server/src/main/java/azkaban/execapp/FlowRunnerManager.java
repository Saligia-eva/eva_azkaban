 ...
 * completed.
 *
 * RunnerManager 是 executorServer 级别的，是 executor 的重要组成部分， 每个 executor 只有一个。
 * 用来统一的进行 workFlow 的控制(包括提交，暂停，取消，重跑等的操作)。还有对当前 executor 进行管理控制。
 *
 * RunnerManager 的构造过程  :
 *    1. 获取 executor 的加载器跟 project 的加载器。
 *    2. 创建 executor 目录跟project 目录分别用来存放当前正在执行的 executor 跟 executor 所调用的 project .
 *    3. 创建用于清理过期的 executor 目录跟 旧版本的 project 目录，还有定期的将结束的 flow 从 running中移交到 finish 中去。
 *
 * Running flow 的提交过程 :
 *    1. 首先从 mysql 的表中获取到指定 id 的已经准备好的 flow
 *       然后根据版本信息，在 executor 中创建执行目录，并将目录注册到 flow 中去。
 *    2. 查看 pipeline ，看是否需要等待其他的 flow 结束， 如果需要等待，则创建一个监控等待其结束。
 *
 *
 */
public class FlowRunnerManager implements EventListener,
  ...
  private File executionDirectory;      // flow 的运行目录
  private File projectDirectory;        // flow 的 project 存放目录
  ...
  // 导入已经存在的 project ， projectId,version
  ...
  private int numThreads = DEFAULT_NUM_EXECUTING_FLOWS; // flow 的线程数量
  private int numJobThreadPerFlow = DEFAULT_FLOW_NUM_JOB_TREADS;  // 每个 flow 中 job 的执行线程数量

  private int threadPoolQueueSize = -1;

  private TrackingThreadPool executorService;    // executorServe 执行线程池
  ...
  /*
  *  主要功能是 :
  *            1. 每分钟清理一次已经结束的 project,从 finishFlow 队列中移除。
  *            2. 每 5 分钟检查清理已过期的旧版本的 project.
  *            3. 每小时检查删除已经过期的 executionId 目录
   */


  private ExecutorLoader executorLoader;          // JDBCExecutorLoader
  private ProjectLoader projectLoader;            // JDBCProjectLoader

  private JobTypeManager jobtypeManager;

  private Props globalProps = null;

  private final Props azkabanProps;    // azkaban 运行时属性 -- 来自配置文件

  private long lastCleanerThreadCheckTime = -1;
  private long executionDirRetention = 1 * 24 * 60 * 60 * 1000;   // executionId 目录的默认生存时间

  // We want to limit the log sizes to about 20 megs
  private String jobLogChunkSize = "5MB";      // log 的大小限制
  private int jobLogNumFiles = 4;              // log 的备份数量

  // If true, jobs will validate proxy user against a list of valid proxy users.
  private boolean validateProxyUser = false;    // proxyUser 可用
  ...
  * 加载已经存在的 project
  ...
  private class CleanerThread extends Thread {
      ...
      while (!shutdown) {
        synchronized (this) {
          try {
            ...
            // Cleanup old stuff.
            // 没两分钟清理一次已经结束的 project
            // 将两分钟前结束的 proejct 从 recentlyFinishedFlows 队列中删除
    ...
    private void cleanOlderProjects() {
      ...
      // 将所有的本地下载的 project 建立一个projectI 跟版本列表的映射
  ...
  /*
  *  根据 execId 来提交一个 flow, flow 的信息已经上传在 execution_flows 中
   */
