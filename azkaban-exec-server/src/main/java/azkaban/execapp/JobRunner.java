...
public class JobRunner extends EventHandler implements Runnable {
  ...
  private String flowOwnUer = null;
  ...
  public void setFlowOwnUser(String user){
    this.flowOwnUer = user;
  }
  public void setFlowOwnGroup(String group){
    this.flowOwnGroup = group;
  }
  public void setFlowLastModifyUser(String user){
    this.flowLastModifyUser = user;
  }
  ...
  public void run() {
    ...
    createLogger();
    logger.info("sucess in thread to run job : " + this.jobId);
    ...
    if (!errorFound && !isKilled()) {
      logger.info("firEvent set : " + this.jobId);
      fireEvent(Event.create(this, Type.JOB_STARTED, new EventData(finalStatus)));
      try {
        logger.info("uploadExecutableNode set : " + this.jobId);
        loader.uploadExecutableNode(node, props);
      } catch (ExecutorManagerException e1) {
        logger.error("Error writing initial node properties");
      }
      logger.info("prepareJob set : " + this.jobId);
      Status prepareStatus = prepareJob();    // 准备环境变量
      logger.info("prepareJob set sucess : " + this.jobId);
    ...
    logger.info("job will be finish : " + this.jobId);
  ...
  private Status prepareJob() throws RuntimeException {
    ...
    synchronized (syncObject) {
       ...
       *  注入 hadoop 环境
       */
      logger.info("Submit user : " + this.flowLastModifyUser + ":" +this.flowOwnGroup + "["+this.flowOwnUer+"]");
      logger.info("Add azkaban propers");
      try {
        if(this.flowOwnGroup != null) {
          HashMap<String, String> envPros = loader.fetchAzkabanEnvByGroup(flowOwnGroup);
          for(String key : envPros.keySet()){
            logger.info("[" + key+"] : " + envPros.get(key));
            props.put(AbstractProcessJob.ENV_PREFIX+key, envPros.get(key));
          }
        }
      } catch (ExecutorManagerException e) {
      ...
      logger.info("Add azkaban propers complate");
      ...
      if(proxyUsers != null){
        ...
        logger.debug("hadoop proxy use set sucess");
      }
      */

      if(this.flowOwnGroup != null){
        defaultProxyUser = this.flowOwnGroup;
        props.put(AbstractProcessJob.ENV_PREFIX+CommonJobProperties.HADOOP_PROXY_USER, defaultProxyUser);
        logger.debug("hadoop proxy use set sucess");
      }
